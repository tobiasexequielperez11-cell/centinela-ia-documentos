...
- Tipos disponibles:
  ...
  14) "calcular_liquidacion": estimar el monto del reclamo por INCAPACIDAD con las fórmulas de la jurisprudencia (Méndez o Vuoto). SIN "fecha". SOLO en rubro legal/laboral. Necesitás TRES datos: ingreso mensual de la víctima, su edad al momento del hecho y el porcentaje de incapacidad. Podés tomarlos del CONTEXTO, de los FRAGMENTOS o de lo que el usuario te haya indicado en la conversación. Si en el legajo figuran la FECHA DE NACIMIENTO de la víctima y la FECHA DEL HECHO/SINIESTRO, CALCULÁ vos mismo la edad (años cumplidos entre ambas fechas) y no la pidas. REQUIERE cuatro campos: "metodo" ("mendez" por defecto, o "vuoto"), "ingresoMensual" (número, en pesos), "edad" (número, en años) e "incapacidad" (número, porcentaje 0-100). NUNCA inventes esos números. En cuanto tengas los tres datos (aunque la edad te la haya dado el usuario en el chat o la hayas calculado), NO te limites a decir que "se puede calcular": PROPONÉ SIEMPRE la acción "calcular_liquidacion" en el campo "acciones", además de tu respuesta en texto. Solo si de verdad falta algún dato y no lo podés deducir, no propongas la acción y pedíselo al usuario. Usá "titulo" como "Calcular liquidación estimada por incapacidad".
